  ...
  Future<void> _poll() async {
    ...
    try {
      final response = await http
          .get(_apiUri('/api/public/player/$currentPairCode'))
...
class AutoWidgetZone extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final scale = max(0.55, min(1.0, min(constraints.maxWidth / 420, constraints.maxHeight / 320)));

        switch (role) {
          case 'header':
            return _buildHeader(scale * weatherScaleFactor.clamp(0.7, 1.5));
          case 'weather':
            return _buildWeather(scale * weatherScaleFactor.clamp(0.7, 1.5));
          case 'bookings':
            return _buildBookings(scale);
          default:
            return const SizedBox();
        }
      },
