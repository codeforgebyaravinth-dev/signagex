...
class PairingResult {
  ...
  const PairingResult({required this.pairCode, required this.backendUrl});
}

class PairingScreen extends StatefulWidget {
  final String backendUrl;
  final String? initialPairCode;
  final void Function(PairingResult result) onPair;

  const PairingScreen({
    super.key,
    required this.backendUrl,
    required this.onPair,
    this.initialPairCode,
  });
...
class _PairingScreenState extends State<PairingScreen> {
  ...
  void initState() {
    super.initState();
    _pairCodeController = TextEditingController(text: widget.initialPairCode ?? '');
  ...
  Future<void> _submit() async {
    ...
    if (pairCode.isEmpty) {
      setState(() => _error = 'Enter a pairing code');
      return;
    }
    if (backendUrl.isEmpty) {
      setState(() => _error = 'Enter a backend URL');
      return;
    }
    setState(() {
      _busy = true;
    ...
    widget.onPair(PairingResult(pairCode: pairCode, backendUrl: backendUrl));
    if (mounted) {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Pair this TV or mobile device to your signage account',
                    textAlign: TextAlign.center,
                  ...
                  TextField(
                    ...
                    decoration: _fieldDecoration(context, 'Backend URL', 'http://192.168.1.10:8000'),
                  ),
                  ...
                  TextField(
                    ...
                    style: const TextStyle(color: Colors.white),
                    decoration: _fieldDecoration(context, 'Pairing Code', 'ABC123'),
                  ...
                  ElevatedButton(
                    ...
                    child: Text(_busy ? 'Connecting...' : 'Start Player'),
                  ),
                  const SizedBox(height: 12),
                  Text(
                    'Tip: for Android TV, install the APK, open it once, enter the pairing code, then keep the device online.',
                    textAlign: TextAlign.center,
...
class _SignagePlayerState extends State<SignagePlayer> with WidgetsBindingObserver {
  ...
  Future<void> _poll() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() => errorMessage = "Could not load content");
      }
    } catch (e) {
  ...
  Future<void> _fetchWeather() async {
    ...
    setState(() => weatherState = "requesting");
  ...
  Future<void> _loadZonePreferences() async {
    ...
    if (savedOrientation != null &&
      ...
      setState(() => orientationOverride = savedOrientation);
    }
  }
  ...
  Widget _buildPlayerScreen() {
    ...
    final rotatedQuarterTurn = rotationDeg == 90 || rotationDeg == 270;

    final renderSurfaceWidth = rotatedQuarterTurn ? viewportSize.height : viewportSize.width;
    final renderSurfaceHeight = rotatedQuarterTurn ? viewportSize.width : viewportSize.height;

    return Scaffold(
      backgroundColor: Colors.black,
      body: Stack(
        children: [
          // Main content with rotation
          Transform.rotate(
            angle: rotationDeg * (3.14159 / 180),
            child: Transform(
              transform: _getRotationMatrix(rotationDeg),
              child: ColorFiltered(
                ...
                child: hasContent
                    ? _buildContent(visibleZones, canvasWidth, canvasHeight)
                    : _buildEmptyScreen(),
              ),
            ),
          ),
  ...
  Matrix4 _getRotationMatrix(int rotation) {
    switch (rotation) {
      case 90:
        return Matrix4.rotationZ(90 * (3.14159 / 180))
          ..translate(-viewportSize.height.toDouble(), 0.0);
      case 180:
        return Matrix4.rotationZ(180 * (3.14159 / 180))
          ..translate(-viewportSize.width.toDouble(), -viewportSize.height.toDouble());
      case 270:
        return Matrix4.rotationZ(270 * (3.14159 / 180))
          ..translate(0.0, -viewportSize.width.toDouble());
      default:
  ...
  Widget _buildContent(
    ...
    if (renderAsAbsolute && !shouldCompact) {
      return _buildAbsoluteLayout(visibleZones, canvasWidth, canvasHeight);
    }

    return _buildGridLayout(visibleZones, canvasWidth, canvasHeight);
  }
  ...
  Widget _buildAbsoluteLayout(
    ...
    return Stack(
      children: zones.map((zone) {
        ...
        final rect = _getZoneRect(zone, canvasWidth, canvasHeight, placement);

        return Positioned(
          left: FractionalOffset(rect['left']! / 100, 0).dx * MediaQuery.of(context).size.width,
          top: FractionalOffset(0, rect['top']! / 100).dy * MediaQuery.of(context).size.height,
          width: (rect['width'] ! / 100) * MediaQuery.of(context).size.width,
          height: (rect['height']! / 100) * MediaQuery.of(context).size.height,
  ...
  Map<String, double> _getZoneRect(
      Map<String, dynamic> zone,
      double canvasWidth,
      double canvasHeight,
      String placement,
      ) {
    final baseLeft = ((zone['x'] ?? 0).toDouble() / canvasWidth) * 100;
    final baseTop = ((zone['y'] ?? 0).toDouble() / canvasHeight) * 100;
    final baseWidth = ((zone['width_px'] ?? canvasWidth).toDouble() / canvasWidth) * 100;
    final baseHeight = ((zone['height_px'] ?? canvasHeight).toDouble() / canvasHeight) * 100;

    final clampedLeft = baseLeft.clamp(0.0, 100.0);
    final clampedTop = baseTop.clamp(0.0, 100.0);
    final clampedWidth = baseWidth.clamp(0.0, 100.0 - clampedLeft);
    final clampedHeight = baseHeight.clamp(0.0, 100.0 - clampedTop);
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          '${zone['role'] ?? 'zone'} · ${zone['width_px'] ?? 0}x${zone['height_px'] ?? 0}',
                                          style: TextStyle(
                                            fontSize: 11,
...
class _MediaSlotState extends State<MediaSlot> {
  ...
  Widget build(BuildContext context) {
    ...
    final item = widget.items[currentIndex];
    final itemType = (item['type'] ?? item['kind'] ?? 'media').toString().toLowerCase();
